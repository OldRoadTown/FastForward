...
| rob64-iq32-registered-scan-head | 0 | 0 | 0 | E060：E059 上寄存 old-u 下一扫描 head，切断 advance 自反馈 |
| rob64-iq32-predecoded-bkpr-thresholds | 0 | 0 | 0 | E061：E060 上按 acnt 预解码固定 ROB BKPR 阈值 |
| rob64-iq32-masked-iq-reservations | 0 | 0 | 0 | E062：E061 上寄存 acnt、在 IQ 边界掩码无效 reservation rank，并直接旋转 scan head |
...
| E060 | `d9b60c1450e5874abcde99a0496c3e09a250c293` | rob64-iq32-registered-scan-head | 6977 | 9947 | 24909 | — | — | — | — | — | — | — | E059 正向 old-u 边界优化：新增 `scan_oh_q`，与 `adv_q` 同拍寄存已经旋转到下一扫描位置的 one-hot head；下一拍 8 项 `iss_q & live_q` 读取不再由 `adv_q` 经过 64-bit rotation 驱动。连续 issued 长度用三层并行 prefix-AND 和 9-bit one-hot length 表达，再编码成 0..8 advance。曾尝试从当前 head 扫 16 项再扣除 `adv_q`，综合形成 35 级 leading-gap 网络、路径恶化至 0.9350ns，已完全丢弃且未提交。最终版本 quick、普通重载 9 seeds、依赖重载 3 seeds、dual/full 均通过，所有 cycles 与 E059 一致；seed 7 普通/依赖/dual/full 为 6977/13034/6739/6168。统一 memory-map + `abc -g simple` 下低/名义/高全局从 0.5250/0.5775/0.6300ns 降至 0.4950/0.5375/0.5800ns，关键路径转为 `ingress in_vld_q → ROB bkpr_r`；IQ allocation 为 0.5225ns，picker/egress 各 0.4950ns。计入 0.045ns uncertainty，同 cycles 的时间代理改善 6.43%，扣除映射总单元 288725→291040（+0.80%）后，时间四次方/面积评分代理仍约 1.29×。名义 >0.400ns 端点为 510（ROB/IQ/picker/egress = 331/123/52/4），下一步应同时优化 BKPR 和 IQ allocation reservation；距离 0.4ns 时钟周期仍有约 0.1375ns 组合余量要消除。 |
| E061 | `847b615cdff5e650928cae0d4665de7777648f05` | rob64-iq32-predecoded-bkpr-thresholds | 6977 | 9947 | 24909 | — | — | — | — | — | — | — | E060 等周期 BKPR 优化：`acnt` 只有 0..4，ROB 并行比较 `occ_base/win_q` 与五组固定阈值，再由 `acnt` 选择结果，避免 `in_vld_q → acnt → 7-bit add → compare` 通用链。保留原 `occ/win` 观测线仅供 testbench 原因统计，功能判定与 E060 逐拍等价。quick、普通重载 9 seeds、依赖重载 3 seeds、dual/full 均通过，所有 cycles 与 E060 一致；seed 7 普通/依赖/dual/full 为 6977/13034/6739/6168。统一 memory-map + `abc -g simple` 下低/名义/高全局从 0.4950/0.5375/0.5800ns 降至 0.4750/0.5225/0.5700ns；BKPR 本身降到 0.4775ns，关键路径转为 `live_q → scan_oh_q`，IQ allocation/picker/egress 均为 0.4950ns。计入 0.045ns uncertainty，同 cycles 的时间代理改善 2.58%；映射总单元 291040→291312（+0.093%），扣除面积后时间四次方评分代理仍约 1.109×。名义 >0.400ns 端点仍为 510，但 >0.355ns 端点从 833 增至 1859，主要是固定阈值复制出的 ROB 比较端点；下一步优先分割 IQ allocator，并继续压缩 scan-head 生成路径。 |
| E062 | `0e311ea5fee2a2d7a492b054608a2ec6413f09ec` | rob64-iq32-masked-iq-reservations | 6977 | 9947 | 24909 | — | — | — | — | — | — | — | E061 等周期控制边界优化：ingress 在捕获 `in_vld_q` 时同步寄存三位 `acnt_q`，使 ROB 距离更新、BKPR 与 IQ reservation 不再从四位 valid popcount 起步；IQ 的四个 `alloc_sel_q` 在写入时按本批 `acnt` 掩码，之后 free-list、metadata 和 age-matrix 消费端直接 OR/one-hot 使用，删除 `pend_cnt_q` 扇出译码；ROB 用已有 `run_oh[0:8]` 直接选择九个固定 scan-head 旋转，绕过 one-hot→binary `adv_n`→64-bit case 的重复编解码。仅做 IQ 掩码时 IQ allocation 虽从 0.4950ns 降至 0.4400ns，但全局映射被 `in_vld_q → win_q` 拉高到 0.5325ns，未作为检查点；加入注册 acnt 后最终低/名义/高全局为 0.4750/0.5050/0.5400ns，关键路径为 `pk_v_q → ROB bkpr_r`（17 级：5 XOR + 6 AND + 4 OR + 1 MUX + 1 NOT）。IQ allocation 为 0.4125ns，IQ ready/picker 为 0.4675ns，scan-head/egress 为 0.4950ns。quick、普通重载 9 seeds、依赖重载 3 seeds、dual/full 均通过，所有 cycles 与 E061 完全一致；seed 7 普通/依赖/dual/full 为 6977/13034/6739/6168。名义 >0.355/>0.400ns 端点从 1859/510 降至 394/333；映射总单元 291312→290575（-0.253%）。计入 0.045ns uncertainty，周期代理从 0.5675ns 降至 0.5500ns（-3.08%），同 cycles 的时间四次方/面积评分代理约 1.136×。仍未达到 0.4ns 周期：组合预算必须压到 0.355ns，当前尚差 0.150ns；下一步优先拆分 `pk_v_q/picked_count → BKPR` 与 egress valid 的连续退休判定。 |
