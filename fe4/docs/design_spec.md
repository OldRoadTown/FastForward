...
> 参数：`REG_FEIN`（FEIN 加寄存级，时序保底）、`WAKE_BYPASS`（预唤醒当拍 pick 开关）
...
- **接收者** = 本拍无自类候选的 FE，且对捐出 lat 通过 3.1 的槽预约检查；每拍最多 2 个偷取（双匹配器，扫描方向相反）；
...
- 若偷取匹配链成为收敛瓶颈，可仅保留单匹配器（st2 恒 0，实测差异 ~0.02）或将 `crit` 优先关闭，逐档回退。
