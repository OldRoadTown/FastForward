...
正式得分为`Score = 1 / (T² × sqrt(Area × Power))`，等价地最小化
`Cost = T⁴ × Area × Power`。`T`是统一评分用例组的最终执行时间，不是时钟
周期或重载吞吐。FE单价约3500um²。

下表只是历史本地吞吐和结构估算，不是正式Score：
...
不能用重载吞吐的倒数冒充统一用例最终`T`，也不能把面积、功耗估算直接当作
签核值。因此原“4FE综合得分约为8FE的1.2~1.3倍”结论撤销；两者必须用完全
相同的正式用例、各自可签核时钟周期、总面积和总功耗重新计算。
...
1. 端口严格按题目命名；FE 数 4（`fwd0..3` / `fwded0..3`），例化官方 fe.v 4 份；
