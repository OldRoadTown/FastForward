...
//
// RTL revision : 4FE-safe-v75
// Experiment   : E075-R32-fifth-stored-credit
// Based on     : E068-R32-dynamic-bkpr-credit
// Changes      : add one conservative registered-state BKPR issue credit
//
...
module ff_rob #(
  ...
  // inspect at most five consecutive entries at old_u; this is a conservative
  ...
  // Credit five only when that entry was already issued before this cycle.
  // This avoids adding another current-cycle picked lookup or consumer.
  assign adv_therm[4] = adv_therm[3] & dist_ge5 & iss_q[cred_i4];

  // Convert each thermometer to a one-hot count (0..5), then select fixed
  ...
  wire [4:0] pop_count_oh = { pop_therm[3],
                             ...
                             ~pop_therm[0] };
  wire [5:0] adv_count_oh = { adv_therm[4],
