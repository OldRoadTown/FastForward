// =============================================================================
// ff_issue - I1 issue stage (8-FE variant)
//
// Reads packet data / dependency data from the ROB (parity-restricted data
// muxes), drives FEIN. dp_data is bypassed from the FEOUT bus when the
...
module ff_issue #(
  ...
  input  wire [NFE*(AW-1)-1:0]   pk_idxh_f,
  // ROB read view
  ...
  generate
    ...
    for (gi = 0; gi < NFE; gi = gi + 1) begin : g_uo
      ...
      assign pk_idxh[gi] = pk_idxh_f[gi*(AW-1) +: (AW-1)];
    end
  endgenerate
  ...
  generate
    for (gf = 0; gf < NFE; gf = gf + 1) begin : g_iss
      ...
      wire [AW-1:0] ridx = {pk_idxh[gf], PRB};
  ...
  generate
    if (REG_FEIN) begin : g_regfe
      ...
      for (gf = 0; gf < NFE; gf = gf + 1) begin : g_co
        ...
        assign issue_idx_f[gf*AW +: AW] = {pk_idxh[gf], PRB2};
      end
    end
  endgenerate
